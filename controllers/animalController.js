...
const getIDAnimal = async (req, res)=>{
...
const putAnimal = async (req, res)=>{
  ...
  const { nombre, sonido } = req.body;
  const params = [nombre, sonido, id];
	const sql = `update tbl_animal set nombre = $1, sonido = $2 where id = $3 returning *`;
  const result = await db.query(sql, params)
...
export { getAnimal, getIDAnimal, postAnimal, putAnimal, deleteAnimal };